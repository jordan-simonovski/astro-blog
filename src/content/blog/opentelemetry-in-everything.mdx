...
<Figure
  src="the-coffee-machine.jpg"
  eager
  alt={
    "The setup: a Gaggia espresso machine with the GaggiMate ESP32 controller wired in, the thing that all of this telemetry comes out of"
  }
/>
...
<video controls muted loop playsinline preload="metadata" width="100%">
  ...
  <a href="/assets/dashboard-demo.mp4">Download the dashboard demo</a> instead.
</video>
...
<Figure
  src="coffee-dashboard.png"
  alt={
    "GaggiMate Espresso dashboard showing boiler temperature, pressure, pump flow, scale weight, disconnect counters and recent shots"
  }
/>
...
<Figure
  src="coffee-trace.png"
  alt={
    "A trace waterfall for a single shot: the parent `shot` span with child spans for pre-infusion, bloom, ramp, hold and decline phases"
  }
/>
...
<Figure
  src="stop-doing-firmware.jpeg"
  alt={
    'STOP DOING FIRMWARE meme: phase changes were not meant to be emitted; statements dreamed up by the utterly deranged, "I need to emit a child span per phase for the brew"'
  }
/>
...
(we do not have the ~~RAM~~ capacity), just running aggregates behind the same mutex, reset at the start
...
<Figure
  src="jaypeg.jpg"
  alt={
    'a pixelated image of Hank Hill saying, "do I look like I know what a jay peg is?"'
  }
/>
...
<Figure
  src="firmware-update.jpeg"
  alt={
    "Serial boot log showing the OpenTelemetry plugin starting and the exporter coming up with metrics=1 traces=1 once WiFi connects"
  }
/>
...
None of it is clever. Clever is how you get paged, or in this case it's how you end up debugging
your coffee machine while trying to get started with your day.
...
<Figure
  src="gaggimate-config.JPG"
  alt={
    "GaggiMate settings page with the OpenTelemetry section: toggles to export metrics and per-shot trace spans"
  }
/>
...
All of this lives in my GaggiMate fork on GitHub. Flash it, point it at your collector, and
trace your own bean water.

<RepoCard repo="jordan-simonovski/gaggimate" />

---

_Next time: hooking in an agent which "roasts" my coffee with the data it reads._
